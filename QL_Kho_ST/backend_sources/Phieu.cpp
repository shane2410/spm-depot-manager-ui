...
PhieuNhap::PhieuNhap() : thanhTien(0.0) {}
PhieuNhap::PhieuNhap(const string &maPhieu, const Date &ngay,
                     const string &maNCC)
    : maPhieuNhap(maPhieu), ngayNhap(ngay), maNCC(maNCC), thanhTien(0.0) {}
string PhieuNhap::getMaNCC() const { return this->maNCC; }
Date PhieuNhap::getNgayNhap() const { return this->ngayNhap; }
string PhieuNhap::getMaPhieuNhap() const { return this->maPhieuNhap; }
double PhieuNhap::getThanhTien() const { return this->thanhTien; }
...
PhieuXuat::PhieuXuat() : thanhTien(0.0) {}
PhieuXuat::PhieuXuat(const string &maPhieu, const Date &ngay,
                     const string &TenKH)
    : maPhieuXuat(maPhieu), ngayXuat(ngay), TenKH(TenKH), thanhTien(0.0) {}

string PhieuXuat::getMaPhieuXuat() const { return this->maPhieuXuat; }
Date PhieuXuat::getNgayXuat() const{ return this->ngayXuat; }
string PhieuXuat::getTenKH() const{ return this->TenKH; }
double PhieuXuat::getThanhTien() const { return this->thanhTien; }
...
void PhieuNhap::tinhThanhtien() {
    double tongTien = 0.0;
    Node<ChitietPhieuNhap> *current = dsCTPN.getHead();
    while (current != nullptr) {
        tongTien += current->data.soLuong * current->data.donGiaNhap;
...
void PhieuXuat::tinhThanhtien() {
    double tongTien = 0.0;
    Node<ChitietPhieuXuat> *current = dsCTPX.getHead();
    while (current != nullptr) {
        tongTien += current->data.soLuong * current->data.donGiaBan;
        current = current->pNext;
    }
    this->thanhTien = tongTien;
}
...
bool PhieuNhap::operator==(const PhieuNhap& other) const {
    return this->maPhieuNhap == other.maPhieuNhap;
}
bool PhieuNhap::operator!=(const PhieuNhap& other) const{
    return !(*this==other);
}
bool PhieuXuat::operator==(const PhieuXuat& other) const {
    return this->maPhieuXuat== other.maPhieuXuat;
}
bool PhieuXuat::operator!=(const PhieuXuat& other) const{
    return !(*this==other);
}

void PhieuNhap::setDSHangNhap(const LinkedList<ChitietPhieuNhap>& ds){
    this->dsCTPN = ds;
}

void PhieuXuat::setDSHangXuat(const LinkedList<ChitietPhieuXuat>& ds){
    this->dsCTPX = ds;
}
