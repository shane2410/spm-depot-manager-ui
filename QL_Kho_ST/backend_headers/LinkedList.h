...
ostream &operator<<(ostream &os, const LinkedList<T> &list) {
    ...
    // mahh, tenhh, donvitinh, giaban, soluongton, hansudung
    int getw[] = {8, 20, 8, 12, 8, 12};
    const int TOTAL_WIDTH =
        getw[0] + getw[1] + getw[2] + getw[3] + getw[4] + getw[5] + 19;

    os << string(TOTAL_WIDTH, '-') << endl;
    os << "| " << left << setw(getw[0]) << "Ma HH"
       << " | " << left << setw(getw[1]) << "Ten Hang Hoa"
       << " | " << left << setw(getw[2]) << "DVT"
       << " | " << right << setw(getw[3]) << "Gia Ban"
       << " | " << right << setw(getw[4]) << "SL Ton"
       << " | " << right << setw(getw[5]) << "Han Su Dung"
       << " |" << endl;
    os << string(TOTAL_WIDTH, '-') << endl;
    ...
    while (current != nullptr) {
        ...
        string remainingText =
            hh.printWrappedString(os, hh.getTenHH(), getw[1]);

        os << " | " << left << setw(getw[2]) << hh.getDonViTinh() << " | "
           << right << setw(getw[3]) << fixed << setprecision(0)
           << hh.getGiaBan() << " | " << right << setw(getw[4])
           << hh.getSoLuongTon() << " | " << right << setw(getw[5] - 7)
           << hh.getHanSuDung() << " |" << endl;
